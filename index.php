...
<!-- I'm going to use simplest way of co-ordinating fields with text field, yes the tables! -->
    <table>
		<tr><td><p1>Username</p1></td><td><input type="text" name="Username" pattern='[0-9]{1,6}' title='123456; Input only numbers' placeholder="Create your username" required/></td></tr>
        <tr><td><p1>First name</p1></td><td><input type="text" name="First_Name" placeholder="Type your first name" required/></td></tr>
		<tr><td><p1>Last name</p1></td><td><input type="text" name="Last_Name" placeholder="Type your last name" required/></td></tr><!-- be careful here, name in the input field will be used as $_POST[$namevalue] while confirming registration -->
        <tr><td><p1>Email address</p1></td><td><input type="email" name="Email" placeholder="Type your email address" required/></td></tr>
		<tr><td><p1>Password</p1></td><td><input type="password" name="Pass_word" placeholder="Create password" required/></td></tr>
        <tr><td><p1>Confirm password</p1></td><td><input type="password" name="confirmpassword" placeholder="Confirm password" required/></td></tr>
        <tr><td><p1>Telephone</p1></td><td><input type='tel'name='Telephone' pattern='[\+]\d{1}[\(]\d{3}[\)]\d{3}[\-]\d{4}' title='Phone Number (Format: +9(999)999-9999)' placeholder="Type your phone number" required/></td></tr>
    ...
    else
    {
        ...
        else
        {
                ...
                $sql = "INSERT INTO `users`(`Username`,`First_Name`, `Last_Name`,`Pass_word`, `Email`,`Telephone`) VALUES ('$Username','$First_Name','$Last_Name','$Pass_word','$Email', '$Telephone')";/* SQL command to insert into database*/
                    ...
                    {
                        echo '<p4>'."Your account is registered successfully!, please <a href='Login.php'>Login</a> to proceed".'</p4>';
